...
Lesson 2 keeps the sine oscillator, audio output, and patch cable from Lesson 1. It adds one new physical part: a pitch-control module. This module makes a voltage that the oscillator interprets as pitch.

```text
+------------------+   patch cable   +---------------------------+   patch cable   +------------------+
| PITCH CONTROL    |---------------->| SINE OSCILLATOR           |---------------->| AUDIO OUTPUT     |
| Pitch knob       |  pitch voltage  | Pitch input               | changing voltage | Audio input    |
| Pitch output     |                 | Sine output               |                | Monitor level    |
+------------------+                 +---------------------------+                +------------------+
```

Each cable carries voltage. The modules at its ends give that voltage a role. The oscillator treats the voltage at `Pitch input` as a pitch setting. The audio-output module treats the voltage at `Audio input` as a signal to deliver to the audio adapter.
...
The pitch-control module uses a common analog-synthesizer convention called one volt per octave. Its pitch knob produces a voltage for the oscillator's pitch input. Raising the knob by one volt doubles the oscillator frequency. Lowering it by one volt halves the oscillator frequency.
...
## The New Cable Changes the Oscillator Before It Makes Sound.

At every sample time, the pitch-control module places its knob voltage on `Pitch output`. The new patch cable carries that voltage to the oscillator's `Pitch input`. The oscillator reads the pitch voltage, calculates its frequency, and places a sine-wave voltage on `Sine output`. The first patch cable then carries that changing voltage to the audio-output module.
...
The [Lesson 2 configuration](../src/lessons/lesson_02.py) starts with the Lesson 1 board, mounts a `PitchControl` module, and patches it to the existing oscillator. The [pitch-control code](../src/synth/pitch_control.py) owns the knob and output jack. The existing [oscillator code](../src/synth/oscillator.py) reads the new input jack.
