...
## You can change waveform and cutoff.

Press `b` to start the sawtooth. Use the right arrow until Oscillator is selected, then use the up and down arrows to choose Sine, Triangle, Square, or Sawtooth with Waveform. Waveform starts at Sawtooth, so the filter begins with a signal that has harmonics to reduce.
...
| Pitch A4 440 Hz |   | Waveform Sawtooth |   | Gain 0.50 |   | Cutoff 12000 Hz |   | To system sound |
| --- | --- | --- | --- | --- | --- | --- | --- | --- |
...
The oscillator retains its Waveform control, and this filter adds one control: Cutoff. Resonance changes behavior near cutoff, so the course introduces it in a later lesson. Press `r` to stop audio and restore Sawtooth, A4 at `440.00 Hz`, Gain at `0.50`, and Cutoff at `12000 Hz`.
